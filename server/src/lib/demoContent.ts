...
// 登入頁「以新人身分體驗」按鈕（server/src/routes/auth.ts 的 POST /auth/new-user）跟
// server/prisma/seed.ts 的示範帳號共用同一份範例內容，改內容只要改這裡一個地方。
export async function seedDemoContentFor(userId: string) {
  ...
  // 「目前第 X 天」是相對「今天」算出來的，用 dayjs 反推開始日期，不管哪天種都會呈現一樣的天數。
  const today = dayjs()
